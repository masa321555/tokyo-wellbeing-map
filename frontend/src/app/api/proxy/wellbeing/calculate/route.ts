...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 必須パラメータの検証
    if (!body.area_id || !body.weights) {
      return NextResponse.json(
        { error: 'area_id and weights are required', requestId },
        { status: 400 }
      );
    }
    ...
    const response = await fetchWithRetry(
      `${API_BASE_URL}/api/v1/wellbeing/calculate/`,
      {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          family_size: body.family_size || 4,
        }),
    ...
    console.log(JSON.stringify({
      requestId,
      endpoint: '/api/proxy/wellbeing/calculate',
      upstream: `${API_BASE_URL}/api/v1/wellbeing/calculate/`,
