...
class HouseholdSimulationRequest(BaseModel):
    """家計シミュレーションリクエスト"""
    area_id: int = Field(..., description="対象エリアID")
...
class LifestyleSimulationRequest(BaseModel):
    """生活利便性シミュレーションリクエスト"""
    current_area_id: int = Field(..., description="現在のエリアID")
    target_area_id: int = Field(..., description="検討中のエリアID")
...
async def simulate_household_budget(
    ...
    if area.housing_data:
        room_rent_map = {
            "1R": area.housing_data[0].rent_1r,
            "1K": area.housing_data[0].rent_1k,
            "1DK": area.housing_data[0].rent_1dk,
            "1LDK": area.housing_data[0].rent_1ldk,
            "2LDK": area.housing_data[0].rent_2ldk,
            "3LDK": area.housing_data[0].rent_3ldk
        }
...
async def simulate_lifestyle_change(
    ...
    転居による生活の変化をシミュレーション
    """
    current_area = db.query(Area).filter(Area.id == request.current_area_id).first()
    target_area = db.query(Area).filter(Area.id == request.target_area_id).first()
...
async def estimate_commute_time(
    from_area_id: int,
    to_station: str,
    db: Session = Depends(get_db)
):
    ...
    通勤時間を推定
    """
    area = db.query(Area).filter(Area.id == from_area_id).first()
...
def _evaluate_lifestyle(area: Area, request: LifestyleSimulationRequest) -> Dict[str, float]:
    ...
    # 公園評価
    if area.park_data:
        park_score = min(100, area.park_data[0].total_parks * 2)
        if area.park_data[0].children_parks:
            park_score += min(20, area.park_data[0].children_parks * 5)
    ...
    if area.school_data:
        ...
        if any(age < 13 for age in request.children_ages):
            school_score = min(100, area.school_data[0].elementary_schools * 10)
        if any(13 <= age < 16 for age in request.children_ages):
            school_score += min(50, area.school_data[0].junior_high_schools * 10)
        evaluation["schools"] = school_score
    
    # 医療評価
    if area.medical_data:
        medical_score = min(100, 
            area.medical_data[0].hospitals * 20 +
            area.medical_data[0].clinics * 2
        )
        if request.children_ages:  # 子供がいる場合
            medical_score += min(30, area.medical_data[0].pediatric_clinics * 10)
...
def _analyze_lifestyle_changes(
    ...
    if request.children_ages:
        if target.childcare_data and current.childcare_data:
            target_waiting = target.childcare_data[0].waiting_children
            current_waiting = current.childcare_data[0].waiting_children
