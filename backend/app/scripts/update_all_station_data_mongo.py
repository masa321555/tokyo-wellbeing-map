#!/usr/bin/env python3
"""
修正された全ての駅・路線情報をMongoDBに更新するスクリプト
"""
import asyncio
import sys
import csv
...
async def update_all_station_data():
    """全ての修正された駅・路線情報をMongoDBに更新"""
    
    # CSVファイルを読み込み（最終版）
    csv_path = "/Users/mitsuimasaharu/Documents/CLI_CODE/tochijihai/tokyo-wellbeing-map/backend/app/data/tokyo_townlist_with_stations_100percent.csv"
    ...
    for ward_name, town_data in data_by_ward.items():
        ...
        if area:
            ...
            print(f"✓ {ward_name}: {with_station}/{total_towns}町に駅情報あり（{round(with_station / total_towns * 100, 1)}%）")
            updated_count += 1
        else:
...
if __name__ == "__main__":
    asyncio.run(update_all_station_data())